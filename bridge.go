...
type BridgeState struct {
	...
	MumbleTotalCount   int
	MumbleChannelUsers map[string]bool
	AutoChan           chan bool
}
...
func discordStatusUpdate(dg *discordgo.Session, host, port string, l *Listener) {
	...
	for {
		...
		if err != nil {
			...
			if curr != l.Bridge.MumbleTotalCount {
				l.Bridge.MumbleTotalCount = curr
			}
			if curr == 0 {
				status = ""
			} else {
				if len(l.Bridge.MumbleChannelUsers) > 0 {
					status = fmt.Sprintf("%v/%v users in Mumble\n", len(l.Bridge.MumbleChannelUsers), curr)
				} else {
...
func AutoBridge(s *discordgo.Session, l *Listener) {
	...
	for {
		...
		time.Sleep(3 * time.Second)
		l.UserCountLock.Lock()
		if !l.Bridge.Connected && l.Bridge.MumbleTotalCount > 0 && len(l.Bridge.DiscordUsers) > 0 {
		...
		if l.Bridge.Connected && l.Bridge.MumbleTotalCount == 0 && len(l.Bridge.DiscordUsers) <= 1 {
